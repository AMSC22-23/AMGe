...
static void BM_FiveLevel(benchmark::State& state) {
	int n = state.range(0);
	Lattice fine(0.0, 0.0, 1.0, 1.0, n+1);
	Multigrid solver(fine, PRE_SMOOTHING_STEPS, POST_SMOOTHING_STEPS, 5);
...
static void BM_SixLevel(benchmark::State& state) {
	int n = state.range(0);
	Lattice fine(0.0, 0.0, 1.0, 1.0, n+1);
	Multigrid solver(fine, PRE_SMOOTHING_STEPS, POST_SMOOTHING_STEPS, 6);
...
static void BM_SevenLevel(benchmark::State& state) {
	int n = state.range(0);
	Lattice fine(0.0, 0.0, 1.0, 1.0, n+1);
	Multigrid solver(fine, PRE_SMOOTHING_STEPS, POST_SMOOTHING_STEPS, 7);
...
BENCHMARK(BM_SixLevel)->Arg(32)->Arg(64)->Arg(128)->Arg(256)->Arg(512)->Arg(1024);
BENCHMARK(BM_SevenLevel)->Arg(64)->Arg(128)->Arg(256)->Arg(512)->Arg(1024);
BENCHMARK_MAIN();
